...
function SidebarProvider({
  ...
  // Keyboard shortcuts: Cmd/Ctrl+B to toggle, ESC to close
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // ESC to close (only if sidebar is open)
      if (event.key === "Escape") {
...
function Sidebar({
  ...
  // Focus trap for desktop when sidebar is open
  React.useEffect(() => {
    if (isMobile || !open || collapsible !== "offcanvas") return
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetHeader className="sr-only">
          ...
          <SheetDescription>
            Main navigation sidebar. Use arrow keys to navigate, ESC to close.
          </SheetDescription>
        </SheetHeader>
        <SheetContent
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2.5 overflow-hidden rounded-lg p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-all duration-150 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent/70 data-[active=true]:text-sidebar-foreground data-[active=true]:font-medium data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
