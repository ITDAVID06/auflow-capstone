...
function SheetContent({
  ...
  children,
  side = "right",
  showCloseIcon = false,       // keep hidden by default
  offsetTop,                   // NEW: number of pixels from the top of the viewport
  style,
  ...props
  ...
  // Merge inline style for top-constrained drawer
  const mergedStyle = {
    ...(offsetTop !== undefined
  ...
  const sideClass =
    side === "right"
      ? offsetTop !== undefined
        ? "right-0 sm:max-w-sm border-l"                // no inset-y-0/h-full; we use inline top+height
        : "inset-y-0 right-0 h-full sm:max-w-sm border-l"
      : side === "left"
      ? offsetTop !== undefined
        ? "left-0 sm:max-w-sm border-r"
        : "inset-y-0 left-0 h-full sm:max-w-sm border-r"
      : side === "top"
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        {showCloseIcon && (
          <SheetPrimitive.Close className="ring-offset-background focus:ring-ring absolute right-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-offset-2">
